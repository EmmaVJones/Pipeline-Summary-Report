...
turbidityWholeRecord <- function(upstreamData, downstreamData, parameter, turbidity99th1, turbidity99th2, time1, time2){
  ...
  # Calculate Baseline Turbidity for 2hr window, skip if no data and just calculate NAs
  if( all( all(is.na(dataWindow$upstream)) , all(is.na(dataWindow$downstream)) )){
    together <- mutate(dataWindow, parameter = parameter, 
    numericDiff = NA, pctDiff = NA, window = NA, turbidity_valid30minuteWindow = NA, upstreamMedian = NA, 
    downstreamMedian = NA, NumericTurbidity_Flag = NA, turbidity_NAs = NA, PercentTurbidity_Flag = NA,
    baselineTurbidity = NA, exceedance = NA, exceedanceType = NA,turbidity_upstreamFlag99th = NA,
    turbidity_upstreamNAs =  NArecords(upstream), turbidity_downstreamFlag99th= NA,
    turbidity_downstreamNAs = NArecords(downstream))
  } else {
    together <- dataWindow %>%
      tq_mutate(
        select     = upstream,
